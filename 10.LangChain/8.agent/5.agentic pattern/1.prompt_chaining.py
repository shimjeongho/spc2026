# 질문 -> 질문 의도 파악 -> 어떤 에이전트 -> 에이전트 응답 -> 질문의 의도 파악 -> 에이전트 응답 -> 최종 응답
...
analysis_prompt = ChatPromptTemplate.from_template(
    """ 
    다음 리서치 결과를 바탕으로 심층 분석 내용을 작성해주세요.
...
report_prompt = ChatPromptTemplate.from_template(
    """ 
    다음 리서치와 분석 내용을 바탕으로 보고서를 작성해주세요.
...
def run_chaining_pipeline(topic):
    ...
    if gate_result.strip().upper() == "FAIL":
...
# topic = "2025년도 생성형 한해동안의 주요 해킹 사례와 보안 기술 동향을 조사해줘."
