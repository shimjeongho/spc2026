...
# question = "alice의 잔액에서 bob에게 10,000원 송금해줘"
"""
[유저] alice의 잔액에서 bob에게 10,000원 송금해줘
[일시정지] send_payment({'recipient': 'bob', 'amount': 10000})

이대로 실행할까요?? (y/n)y
[최종결론] Alice의 잔액에서 Bob에게 10,000원이 송금되었습니다.
"""
"""
[유저] alice의 잔액에서 bob에게 10,000원 송금해줘
[일시정지] send_payment({'recipient': 'bob', 'amount': 10000})
...
# question = "alice의 잔액과 bob의 잔액을 조회해서, 돈이 많은 사람이 적은사람에게 몰빵해주기"
"""
[유저] alice의 잔액과 bob의 잔액을 조회해서, 돈이 많은 사람이 적은사람에게 몰빵해주기
[일시정지] get_balance({'account': 'alice'})
[일시정지] get_balance({'account': 'bob'})
[일시정지] send_payment({'recipient': 'bob', 'amount': 500000})

이대로 실행할까요?? (y/n)y 
[최종결론] Alice의 잔액은 1,000,000원이었고, Bob의 잔액은 500,000원이었습니다. Alice가 Bob에게 500,000원을 송금하여, 이제 Bob의 잔액이 1,000,000원으로 증가했습니다.
"""
"""
[유저] alice의 잔액과 bob의 잔액을 조회해서, 돈이 많은 사람이 적은사람에게 몰빵해주기
[일시정지] get_balance({'account': 'alice'})
